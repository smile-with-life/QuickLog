...
inline const LogLevel Logger::getAutoFlushLevel() const
{
...
inline void Logger::setFormat(LogFormat::ptr format)
{
    for (auto& iter : m_appenderList)
    {
        iter->setFormat(format->clone());
    }
}


inline void Logger::setPattern(std::string& pattern)
{
    for (auto& iter : m_appenderList)
    {
