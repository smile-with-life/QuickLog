...
QuickLog库的使用非常简单，只需要下载源代码，然后把QuickLog2.0/include目录下的文件拷贝到我们的工程下，在工程中包含相应的头文件即可。
...
int main()
{ 
    auto log = create("console_logger"); 
...
1. 上面的示例代码中,使用了create函数进行自定义logger的创建
2. "console_logger"，指定了日志输出器的名称
3. 日志输出器默认为有锁版本的控制台输出
4. 日志格式与registry的默认格式相同
5. 日志等级与registry的默认等级相同
...
int main()
{ 
    auto log_always = create<console_st>("console_logger_always",ColorMode::always); 
    auto log_automatic = create<console_st>("console_logger_automatic", ColorMode::automatic);
    auto log_never = create<console_st>("console_logger_never", ColorMode::never); 
...
int main()
{ 
    auto log = create<txt_st>("console_logger","test.txt");
...
`std::make_shared<LogForamt> ("{0} {1} [{4}] line[{5}] {6}",TimeType::UTC);`

`std::make_shared<LogForamt> ("{0} {1} [{4}] line[{5}] {6}",TimeType::Local);`
...
| void check_logger(std::string& logger_name);                 | //进行检查，当发现logger已经注册过，抛出异常 |
...
| void setDefaultPattern(const std::string pattern);//设置默认logger的pattern | //设置默认logger的pattern                    |
